$(function(){
    var inputs = { type: $('#item-type select'), host: $('#item-host input'), port: $('#item-port input'), username: $('#item-username input'), password: $('#item-password input'), database: $('#item-database input'), arguments: $('#item-arguments input') };
    ...
    var TYPE_SPECIFIC_OPTIONS = {
        ...
        'db2': {
            description: { 
                'database': _('<em>For DB2, you must manually specify a database name.</em>')
            },
    ...
    function applyTypeSpecificOptions() {
        ...
        if(settings) {
            ...
            if(settings.showAdditionalJDBC){
                $("#item-arguments").show();
            } else {
                $("#item-arguments").hide();
            }
    ...
    $('#item-spl-ctrl_fetch_catalogs button').click(function(e) {
        ...
        var arguments = inputs.arguments.val(); if(arguments) params.arguments = arguments;
