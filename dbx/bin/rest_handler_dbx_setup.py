...
class DBXSetupEndpoint(SetupEndpoint):
    ...
    def process_edit(self, output, props):
        ...
        if id == 'java':
            ...
            entityPath = entity.getBasePath()
            entityPath += '/' + splunk.entity.quoteEntity(self.getJavaBridgeServerScript())
            if script_disabled:
                logger.info("Enabling scripted input for java bridge server: %s", entityPath)
                splunk.entity.controlEntity("enable", "%s/enable" % entityPath,
    ...
    def getJavaBridgeServerScript(self):
        return os.path.join(".", "bin", "jbridge_server.py")
