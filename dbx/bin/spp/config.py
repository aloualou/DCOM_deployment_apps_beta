...
class EntityEndpoint(ConfigEndpoint):
    ...
    def handleList(self, output):
        ...
        for name, settings in config.items():
            ...
            # DBX-289
            self.update_enablelinks(output[name])
    ...
    # DBX=289
    def update_enablelinks(self, confItem):
        if hasattr(confItem, "addCustomAction"):
            disabled = str(confItem.get("disabled", False)).lower()
            if disabled in ["0", "false"]:
                confItem.addCustomAction("disable")
            else:
                confItem.addCustomAction("enable")
    ...
    # DBX-303 power user to create dbmon
    def writeConf(self, confName, stanzaName, settingsDict):
        namespace = self.namespace if hasattr(self, "namespace") else "dbx" 
        entity = en.Entity("configs/conf-%s" % confName, stanzaName, namespace=namespace, owner="nobody")
        entity.properties.update(settingsDict)
