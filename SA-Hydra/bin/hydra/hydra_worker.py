...
class HydraWorker(ModularInput):
    ...
    def isJobExpired(self, job_id, job_tuple, metadata):
        ...
        if expiration_time < cur_time:
            self.logger.debug("[isJobExpired] expired job=%s create_time=%s expiration_period=%s cur_time=%s task=%s",
                              job_id, job_tuple.create_time, job_tuple.expiration_period, cur_time, job_tuple.task)
    ...
    def getJob(self):
        ...
        while True:
            ...
            if self.isJobExpired(job_id, job_tuple, metadata):
                self.logger.error("[getJob] job=%s of task=%s has expired and will not be run", job_id, job_tuple.task)
                is_atomic = False
                if job_tuple.task in metadata.get("atomic_tasks", []):
                    ...
                    is_atomic = True
                if is_atomic:
                    self.logger.error("[getJob] reporting expired atomic job as a job failure for job=%s of task=%s",
                                      job_id, job_tuple.task)
    ...
    def assignJobToHandler(self, job):
        ...
        #Execute Job
        self.logger.info("Assigning job=%s of task=%s with config=%s to handler=%s", job_id, job_tuple.task, config,
                         handler)
        job_start_time = time.time()
        if handler.run(session, config, job_tuple.create_time, job_tuple.last_time):
            self.logger.info("Successfully completed job={0} of task={1}".format(job_id, job_tuple.task))
