...
class HydraCacheManager(object):
    ...
    def set_cache(self, name, value, expiration=None):
        """
        Sets the cache value for a particular item, threadsafe
        """
...
# Utilities & Globals
#===============================================================================
logname = "hydra_gateway.log"
service_logger = setupLogger(logger=None, log_format='%(asctime)s %(levelname)s [HydraWSGI:%(process)d] %(message)s',
                             level=logging.INFO, log_name=logname, logger_name="hydra-gateway")
logname = "hydra_access.log"
access_logger = setupLogger(logger=None, log_format='%(asctime)s %(levelname)s %(message)s', level=logging.INFO,
                            log_name=logname, logger_name="hydra-access")
...
def get_pop_job(environ, start_response):
    ...
    try:
        ...
        if get_args is not None and get_args != '':
            ...
            for arg in arg_list:
                kvpair = arg.split("=")
                if kvpair[0] == "block" and len(kvpair) == 2:
                    if kvpair[1] == "0":
                        block = False
                    else:
                        block = True
                else:
                    service_logger.warn("[JobPopper] got unexpected get_arg=%s", kvpair)
...
def bootstrap_web_service(port=8008, service_log_level="INFO", access_log_level="INFO"):
    """
    Start up the hydra web service from conf file defitions
    ...
    if service_log_level not in ["DEBUG", "INFO", "WARN", "WARNING", "ERROR"]:
        ...
        service_logger.warning("unrecognizable configured service log level: %s, resetting log level to INFO",
                               service_log_level)
    else:
    ...
    if access_log_level not in ["DEBUG", "INFO", "WARN", "WARNING", "ERROR"]:
        ...
        access_logger.warning("unrecognizable configured access log level: %s, resetting log level to INFO",
                              access_log_level)
    else:
    ...
    try:
        ...
        if e.errno != 61:
            service_logger.warning(
                "got unexpected socket error when checking port availability, will attempt to bootstrap gateway anyway. socket error: %s",
                str(e))
    ...
    try:
        ...
        server.ssl_version = PROTOCOL_SSLv23_NO23
    ...
    #Attempt to recover any old cache
    try:
        recover_cache()
    except Exception as e:
