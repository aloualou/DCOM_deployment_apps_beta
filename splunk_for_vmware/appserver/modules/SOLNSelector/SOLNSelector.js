...
Splunk.Module.SOLNSelector = $.klass(Splunk.Module.DispatchingModule,{
	...
	validateInput : function(inputStr) {
		...
		if(this.readStructures==="1"){
			...
			var customEndPtUrl=  Splunk.util.make_url('custom', 'splunk_for_vmware', 'read_structures_service', 'splunk_for_vmware','validate_input') ;
			$.ajax({
				type: "GET",
				url:customEndPtUrl,
				data:input,
		...
		/*	if(this.hostVm!=null){
			...
			for( var key in this.selections){
				if(this.readStructures!=="1"){
					...
					input['datatype']=this.types[0];
					var idKeys={};
					var customEndPtUrl=  Splunk.util.make_url('custom', 'splunk_for_vmware', 'read_structures_service', 'splunk_for_vmware','get_id_keys');
					var ajxRqst= $.ajax({
						type: "GET",
						url: customEndPtUrl,
						data:input,
	...
	findPossibleSuggestions : function(searchString) {
		...
		if(this.readStructures==="1"){
			...
			input['datatype']=this.types[0];
			var customEndPtUrl=  Splunk.util.make_url('custom', 'splunk_for_vmware', 'read_structures_service', 'splunk_for_vmware','find_possible_matches');
			$.ajax({
				type: "GET",
				url:customEndPtUrl,
				data:input,
	...
	findMatches : function(T, searchString, matches) {
		...
		if(this.readStructures==="1"){
			...
			input['datatype']=this.types[0];
			totalCount=0;
			var customEndPtUrl=  Splunk.util.make_url('custom', 'splunk_for_vmware', 'read_structures_service', 'splunk_for_vmware','find_matches');
			$.ajax({
				type: "GET",
				url:customEndPtUrl,
				data:input,
		...
		if (this.firstLoad) {
			...
			for (ii = 0; ii< this.types.length; ii++) {
				...
				if (urlVal && urlVal!=='(moid="" AND host="")') {
					...
					if(this.readStructures==="1"){
						selectedKey="";
						customEndPtUrl=  Splunk.util.make_url('custom', 'splunk_for_vmware', 'read_structures_service', 'splunk_for_vmware','get_selected_key');
						$.ajax({
								type: "GET",
								url:customEndPtUrl,
								data:input,
