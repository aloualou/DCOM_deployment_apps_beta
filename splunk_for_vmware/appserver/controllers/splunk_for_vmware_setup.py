...
def isValidAdmin(user=None, session_key=None):
	...
	try:
		...
		if 'splunk_vmware_admin' in roles or 'admin' in roles:
...
class splunk_for_vmware_setup(controllers.BaseController):
	...
	def show_collection_setup(self, **kwargs):
		...
		return self.render_template('/splunk_for_vmware:/templates/collection_setup.html', dict(conf_data=conf_data))
	...
	def validate_collection_node(self, app, action, **kwargs):
		...
		try:
			...
			if remote_session_key is None:
				...
				required_apps = ["SA-Utils", "SA-Hydra", "Splunk_TA_vmware"]
				installed_count = 0
				installed_apps = []
				for app in apps:
	...
	def validate_vcenter_forwarder(self, app, action, **kwargs):
		...
		try:
			...
			if remote_session_key is None:
				...
				#Okay credentials are good, now we can check that the apps are there
				apps = App.all(host_path=vc_host_path, sessionKey=remote_session_key)
	...
	def validate_unmanaged_host(self, app, action, **kwargs):
		...
		if not host_username or not host_password:
			#No username/password provided so it must exist in the conf
			if not host_stanza:
				logger.error("host={0} with stanza={1} passed to validate_vcenter does not exist and username/password unspecified, cannot validate nothing!".format(host, name))
	...
	def save_collection_node(self, app, action, **kwargs):
		...
		if node_stanza.user != username:
			...
			if stored_cred:
				...
				logger.info("Deleting outmoded credential={0}".format(str(stored_cred)))
				if not stored_cred.passive_delete():
					logger.error("Could not delete outmoded credential={0} it may linger".format(str(stored_cred)))
		if password:
	...
	def delete_collection_node(self, app, action, node_path, **kwargs):
		...
		if node_stanza:
			...
			if stored_cred:
				...
				logger.info("Deleting obsolete credential={0}".format(str(stored_cred)))
				if not stored_cred.passive_delete():
					logger.error("Could not delete obsolete credential={0} it may linger".format(str(stored_cred)))
			else:
				node_password = None
			if not node_stanza.passive_delete():
			...
			try:
				...
				for cred in creds:
					if not cred.passive_delete():
						logger.error("Problem deleteing credential on node={0} for credential={1}".format(node_path, str(cred)))
			except ServerNotFoundError:
	...
	def save_vcenter(self, app, action, **kwargs):
		...
		if vc_splunk_uri is None or vc_splunk_username is None:
			...
			if forwarder_stanza:
				...
				logger.info("Forwarder stanza exists. Updating infomation for vc=%s, creating new forwarder stanza with vc_splunk_uri=%s and username=%s...", vc, vc_splunk_uri, vc_splunk_username)
				...
				if not forwarder_stanza.passive_save():
					logger.error("problem updating the splunk forwarder stanza for vc=%s and vc_splunk_uri=%s and username=%s, stanza will have to be regenerated", vc, vc_splunk_uri, vc_splunk_username)
			else:
			...
			if not forwarder_stanza:
				logger.info("No stored forwarder infomation for vc=%s, creating new forwarder stanza with vc_splunk_uri=%s and username=%s...", vc, vc_splunk_uri, vc_splunk_username)
				...
				forwarder_stanza.vc_collect_logs = vc_collect_logsflag
				if not forwarder_stanza.passive_save():
					logger.error("problem saving splunk forwarder stanza for vc=%s and vc_splunk_uri=%s and username=%s, stanza will have to be regenerated", vc, vc_splunk_uri, vc_splunk_username)
			else:
				logger.info("Forwarder stanza exists. Updating infomation for vc=%s, creating new forwarder stanza with vc_splunk_uri=%s and username=%s...", vc, vc_splunk_uri, vc_splunk_username)
				forwarder_stanza.host = vc_splunk_uri
				forwarder_stanza.user = vc_splunk_username
			...
			if not spl_stored_cred:
				logger.info("No stored forwarder credentials for vc=%s, creating new stored credential with vc_splunk_uri=%s and username=%s...", vc, vc_splunk_uri, vc_splunk_username)
				...
				if not vc_splunk_password or spl_stored_cred.clear_password == vc_splunk_password:
					...
					logger.info("Password for splunk forwarder on vc=%s with vc_splunk_uri=%s and username=%s changed, updating it secure storage", vc, vc_splunk_uri, vc_splunk_username)
	...
	def delete_vcenter(self, app, action, name):
		...
		if not syslog_stanza:
			...
			if not syslog_stanza.passive_delete():
				logger.error("Could not delete obsolete credential={0} it may linger".format(str(syslog_stanza)))
				raise VMwareSetupError(status="500", message="Failed to clear syslog configuration: {0}")
