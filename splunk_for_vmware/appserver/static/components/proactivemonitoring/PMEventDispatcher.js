	...
	function (_, $, Backbone) {
		...
		dispatcher.on("node:drill", function(node) {
			if (node.type === "RootFolder") {
				...
				Splunk.util.redirect_to("/app/splunk_for_vmware/home_proactive_monitoring");
			}
			else {
				...
				//Drill baby drill!
				Splunk.util.redirect_to("/custom/splunk_for_vmware/vmware_redirector/splunk_for_vmware/redirect", query_string_args);
			}
		});
