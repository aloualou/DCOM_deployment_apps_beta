...
(function() {
    ...
    // Signal that we are in an Advanced XML context.
    // This means notably that full RequireJS is not available (only Almond is).
    window.SplunkLicenseMonitor = window.SplunkLicenseMonitor || {};
    window.SplunkLicenseMonitor.isAdvancedXml = true;
    
    // Perform license check
    loadScriptAsync('/en-US/static/app/splunk_for_vmware/license_check.js');
})();
...
if( $('#AppBar_0_0_0').size() > 0 ){
	...
	if(helpLink!==undefined){
		helpLink = helpLink.replace('%5Bsplunk_for_vmware%3A', '%5BVMW%3A');
		// update help link
