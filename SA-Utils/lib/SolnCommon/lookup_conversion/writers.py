...
class MultipleLookupWriter(Writer):
    ...
    def _move_lookup(self, key_field, output_filename, dont_use_rest_for_file_update=False):
        ...
        if key_field in self._lookups.keys():
            ...
            transform = get_lookup_transform(tgt_lookup_name, namespace, owner, self._session_key)
            tgt_lookup_path = None
            if transform:
                tgt_lookup_path = get_lookup_table_location(transform=transform, key=self._session_key, fullpath=False)
                logger.info('RETRIEVED: path for target lookup name: %s', tgt_lookup_path)
            
            if output_filename is not None and os.path.isfile(output_filename) and tgt_lookup_path:
                logger.info('LOOKUP_GEN:temporary file retrieved: file=%s', output_filename)
                logger.info('UPDATING: Target lookup table: %s', tgt_lookup_path)
                ...
                if dont_use_rest_for_file_update or os.name in ['nt', 'os2']:
                    ...
                    dst_lookup_path = make_splunkhome_path(['etc', 'apps', namespace, "lookups", tgt_lookup_path])
                    # Path to the temporary location for the old lookup table, 
                    # to be deleted. Timestamp plus random integer is used to 
                    # attempt uniqueness.
                    tmp_lookup_fname = "_".join([tgt_lookup_path, str(int(time.time())), str(random.randint(0,255))])
                    tmp_lookup_path = make_splunkhome_path(['etc', 'apps', namespace, "lookups", tmp_lookup_fname])
                    ...
                    was_successful = update_lookup_table(output_filename, tgt_lookup_path, namespace, owner, self._session_key)

                if was_successful:
                    logger.info('UPDATED: Target lookup table: %s', tgt_lookup_path)
                else:
                    logger.error('FAILURE: %s: %s', LookupConversionErrors.ERR_LOOKUP_CREATION_FAILED, tgt_lookup_path)
            else:
